...
// Carousel de boas-vindas do app NATIVO (first-run, pré-cadastro). Rota pública
// e estaticamente exportável (client puro, sem server/rota dinâmica). Vive
// SEPARADO do /comecar da web. A WEB não usa esta rota no fluxo — só abre se
// digitada (foi adicionada a isPublicPage no proxy.ts só p/ teste no localhost).
//
// Nesta fatia: só os 3 slides + CTAs interinos. As próximas fatias acrescentam o
// quiz sob este mesmo guarda-chuva.

import { useRef, useState } from 'react';
import { useRouter } from 'next/navigation';

const ACCENT = '#5B5BD6';
const ACCENT_DARK = '#4A4AC4'; // links/hover p/ contraste AA
const BG = '#F7F7F5';
const INK = '#1A1A1A';
const INK_2 = '#6B6B6B';
...
export default function InicioCarousel() {
  ...
  const [drag, setDrag] = useState(0); // translateX ao vivo durante o swipe

  // Sai do carousel pelo CTA: marca a intro como vista (não repetir no relaunch)
  // e navega pro destino. A flag só seta AQUI — "Pular" apenas adianta o slide.
  function exit(dest: string) {
    try {
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    // Resistência nas bordas (não há slide além das pontas).
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* "Pular" só adianta pro slide final (CTAs). Some no último slide. */}
        <button
          type="button"
          onClick={() => goTo(LAST)}
          style={{
        ...
        >
          {SLIDES.map((s, i) => (
            ...
            >
              ...
              <h1
                style={{
                  font: '800 26px/1.25 Nunito, sans-serif',
                  color: INK,
                  margin: '0 0 14px',
                  letterSpacing: '-.4px',
                  maxWidth: 320,
              ...
              <p
                style={{
                  font: '500 16px/1.5 Nunito, sans-serif',
                  color: INK_2,
                  margin: 0,
                  maxWidth: 300,
                }}
