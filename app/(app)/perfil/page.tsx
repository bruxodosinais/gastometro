      ...
      <main className="max-w-lg md:max-w-[1100px] mx-auto px-4 md:px-8 pt-8 pb-6">
        ...
        <div>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {!WHATSAPP_ENABLED ? (
              <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--r)', padding: 18 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--accent-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>💬</div>
                  <div>
                    <p style={{ fontSize: 14, fontWeight: 800, color: 'var(--text)', margin: 0 }}>Bot WhatsApp — Em breve</p>
                    <span style={{ display: 'inline-block', marginTop: 4, padding: '2px 8px', borderRadius: 999, background: 'var(--accent)', color: '#fff', fontSize: 10, fontWeight: 900, letterSpacing: '0.04em' }}>Exclusivo Pro</span>
                  </div>
                </div>
              ...
              <UpgradeBanner message="Vinculação de WhatsApp é exclusiva do plano Pro" feature="whatsapp" />
            ) : whatsappVerified && whatsappPhone ? (
              <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--r)', padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 15, fontWeight: 800, color: 'var(--text)', margin: 0 }}>{formatStoredPhone(whatsappPhone)}</p>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 4, padding: '2px 8px', borderRadius: 999, background: 'var(--green-bg)', color: 'var(--green-text)', fontSize: 11, fontWeight: 800 }}>
                      <Check size={11} /> Conectado
                    </span>
                  </div>
                </div>
                <button type="button" onClick={handleUnlinkWhatsapp} disabled={waUnlinking} style={{ width: '100%', boxSizing: 'border-box', background: 'transparent', border: '1.5px solid var(--red)', borderRadius: 'var(--r-sm)', padding: 12, fontSize: 13, fontWeight: 800, color: 'var(--red)', cursor: waUnlinking ? 'default' : 'pointer', opacity: waUnlinking ? 0.6 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              <div style={{ background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--r)', padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                  ...
                  <p style={{ fontSize: 13, fontWeight: 800, color: 'var(--text)', margin: 0 }}>Vincular WhatsApp</p>
                </div>
                ...
                <label style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Número com DDD</label>
                <input type="tel" inputMode="numeric" value={waInput} onChange={(e) => setWaInput(maskBrazilPhone(e.target.value))} placeholder="(11) 99999-9999" disabled={waOtpSent} style={{ width: '100%', boxSizing: 'border-box', background: waOtpSent ? 'var(--bg)' : 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--r-sm)', padding: '12px 14px', fontSize: 16, fontWeight: 700, color: 'var(--text)', outline: 'none', opacity: waOtpSent ? 0.6 : 1 }} />
                {!waOtpSent ? (
                  <button type="button" onClick={handleSendWhatsappOtp} disabled={waSending} style={{ marginTop: 12, width: '100%', boxSizing: 'border-box', background: 'var(--accent)', color: 'white', border: 'none', borderRadius: 'var(--r-sm)', padding: 14, fontSize: 14, fontWeight: 800, cursor: waSending ? 'default' : 'pointer', opacity: waSending ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                    {waSending ? <Loader2 size={16} className="animate-spin" /> : null}
                    {waSending ? 'Enviando…' : 'Enviar código'}
                  </button>
                ) : (
                  <>
                    <label style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', margin: '14px 0 6px' }}>Código de verificação</label>
                    <input type="text" inputMode="numeric" value={waOtp} onChange={(e) => setWaOtp(e.target.value.replace(/\D/g, '').slice(0, 6))} placeholder="000000" maxLength={6} style={{ width: '100%', boxSizing: 'border-box', background: 'var(--surface)', border: '1.5px solid var(--border)', borderRadius: 'var(--r-sm)', padding: '12px 14px', fontSize: 16, fontWeight: 800, color: 'var(--text)', letterSpacing: '0.3em', textAlign: 'center', outline: 'none' }} />
                    <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                      <button type="button" onClick={() => { setWaOtpSent(false); setWaOtp(''); setWaError(''); setWaSuccess(''); }} disabled={waVerifying} style={{ flex: 1, padding: '12px 0', borderRadius: 'var(--r-sm)', border: '1.5px solid var(--border)', background: 'var(--surface)', fontSize: 13, fontWeight: 600, color: 'var(--text-2)', cursor: waVerifying ? 'default' : 'pointer', opacity: waVerifying ? 0.6 : 1 }}>Trocar número</button>
                      <button type="button" onClick={handleVerifyWhatsappOtp} disabled={waVerifying} style={{ flex: 1, padding: '12px 0', borderRadius: 'var(--r-sm)', background: 'var(--accent)', border: 'none', fontSize: 13, fontWeight: 800, color: 'white', cursor: waVerifying ? 'default' : 'pointer', opacity: waVerifying ? 0.7 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                ...
                {waError && <p style={{ color: 'var(--red)', fontSize: 12, background: 'var(--red-bg)', borderRadius: 'var(--r-sm)', padding: '10px 14px', textAlign: 'center', marginTop: 12, marginBottom: 0 }}>{waError}</p>}
                {waSuccess && !waError && <p style={{ color: 'var(--green-text)', fontSize: 12, background: 'var(--green-bg)', borderRadius: 'var(--r-sm)', padding: '10px 14px', textAlign: 'center', marginTop: 12, marginBottom: 0 }}>{waSuccess}</p>}
              </div>
            )}
          </div>
